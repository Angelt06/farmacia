<template>
  ...
  <div class="mt-2">
    <div class="flex justify-end">
      <router-link class="flex items-center duration-200 border-l-4" :to="`/medicamentos/${useRoute().params.medicamentoId}/descuentos/crear`">
        <button
...
const titulo = ref("Listado de descuentos");
...
const consultarDescuentos = async () => {
  ...
  response.data.forEach((descuento) => {
    const esPorcentaje = descuento.id % 2 === 0;

    descuentos.value.push([
      descuento.id,
      esPorcentaje ? parseInt(Math.random() * 50) : parseInt(Math.random() * 10000),
      esPorcentaje ? 'Si' : 'No',
      new Date().toLocaleDateString(),
      new Date().addDays(descuento.userId).toLocaleDateString(),
      `<a href="/medicamentos/${medicamento.value.id}/descuentos/${descuento.id}/editar" class="text-indigo-600 hover:text-indigo-900">Editar</a>`,
    ]);
  });
};
